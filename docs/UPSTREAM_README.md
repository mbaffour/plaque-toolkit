# Plaque Size Tool — original upstream manual

> This is the **original, unmodified manual** for the upstream Plaque Size Tool
> (Trofimova & Jaschke, *Virology* 2021), preserved verbatim for reference and citation.
> For the Plaque Toolkit that wraps it, start at the [project README](../README.md); for the
> focused size-CLI reference see [PLAQUE_SIZE_TOOL.md](PLAQUE_SIZE_TOOL.md). Note: the original
> install-via-`pip` instructions below describe the upstream package, not this toolkit — to set
> up *this* toolkit follow [INSTALL.md](INSTALL.md).
